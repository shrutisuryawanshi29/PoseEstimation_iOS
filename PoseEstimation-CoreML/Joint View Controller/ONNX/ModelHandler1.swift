...
//
//  Created by Amirthavarshini Dhanavel on 10/10/23.
...
typealias FileInfo1 = (name: String, extension: String)

enum OrtModelError1: Error {
    case error(_ message: String)
}

class ModelHandler1: NSObject {
    // MARK: - Inference Properties
    
    let threadCount1: Int32
    let threshold1: Float = 0.5
    let threadCountLimit1 = 10
    ...
    init?(modelFileInfo: FileInfo1, threadCount: Int32 = 1) {
    ...
    /*func createRandomArray(size: Int) -> [Float] {
     var array = [Float]()
     for i in 0..<size {
     array.append(random.uniform(in: 0.0...100.0))
     }
     return array
     }*/
    ...
    func runModel1(onFrame pixelBuffer: CVPixelBuffer) throws {
        ...
        /*guard let rgbData = rgbDataFromBuffer(
         scaledPixelBuffer,
         byteCount: batchSize * inputChannels * inputHeight * inputWidth
         ) else {
        ...
        let inputShape: [NSNumber] = [batchSize as NSNumber,
                                      ...
                                      inputs as NSNumber]
        //let data_2d: [Float] = (0..<34).map { _ in Float.random(in: 0..<1) }
        /*let data_2d = [[33.4,33.4,33.4,33.4,33.4,
         ...
         let numRows = data_2d.count
         let numColumns = data_2d.first?.count ?? 0
         print("row:\(numRows)")
         print("column:\(numColumns)")
         
         print("inputShape \(inputShape)")*/
        
        do{
            //let inputName = try! session.getInputName(at: 0)
            //          let inputData = Data(copyingBufferOf: Array(repeating: Float.random(in: 0.0..<1.0), count: 1 * 34 * MemoryLayout<Float>.stride))
            //            //var ortInputs: [String: ONNXValue] = [inputName: .tensor(inputData)]
            ...
            var inputData = [Float](repeating: 0.0, count: inputShape.reduce(1, *))
            ...
            //var ortInputs: [String: ORTValue] = [inputName: inputVal]
            print("created input")
            //      print("input tensor is \(inputTensor)")
            //            let outputs = try! session.run(inputs: inputTensor, completion: { outputs, _ in
            //                //if let outputName = try? session.getOutputName(at: 0) {
            //                    if let outputData = try? outputs[outputName]?.tensor {
            //                        let outputShape = outputData.shape
            //                        print(outputShape)
            //                    }
            //                }
            //            })
            ...
            //                                          outputNames: ["2600","onnx::MatMul_2593"],
            //                                          runOptions: nil)
        ...
        catch{print("error in running data")}
        
        //formatRTMPoseOutput(simccX: rawOutputValue, outputArr: outputArr, simccY: rawOutputValue_1, outputArr_1: outputArr_1)
     ...
     */
    /*private func loadLabels(fileInfo: FileInfo1) -> [String] {
     ...
     *//*
        func parsePoseYOLOv8(results: [VNCoreMLFeatureValueObservation]) -> [Prediction] {
        ...
        let w = outputArray[(2 * gridWidth) + j].floatValue
        let h = outputArray[(3 * gridWidth) + j].floatValue
        x = x -  (w / 2)
        y = y -  (h / 2)
...
/*[[92.2],[22.2],[20.2],[22.2],[29.2],
 [21.2],[24.2],[22.2],[22.2],[22.2],
 [22.2],[22.2],[22.2],[22.2],[82.2],
 [22.2],[52.2],[62.2],[22.2],[22.2],
 [28.2],[28.2],[22.2],[27.2],[22.2],
 [42.2],[22.2],[82.2],[22.2],[62.2],
 [22.2],[22.2],[24.2],[22.2]]*/
