...
class HumanBodySkeletonRenderer: NSObject {
    ...
    public var nodeJointDict: [HumanKeypointName: SCNNode] = [:]
    ...
    public var PAMJointArr: [HumanKeypointName] = [.Pelvis,
        .R_Hip,
        .R_Knee,
        .R_Ankle,
        .L_Hip,
    ...
    func relate3DSkeletonProportionToImagePlane(observation: [HumanKeypointName : SCNNode]) -> Float {

        var returnScale: Float = 1.0

        guard
            let centerShoulderNode = self.nodeJointDict[.Neck],
            let spineNode = self.nodeJointDict[.Pelvis]
        else {
            return returnScale
        }
        ...
        do {
            let pointCenterShoulder2D = observation[.Neck]?.simdPosition
            let pointSpine2D = observation[.Pelvis]?.simdPosition
            let distance = simd_distance(
                simd_float2(
                    x: Float(pointCenterShoulder2D?.x ?? 0),
                    y: Float(pointCenterShoulder2D?.y ?? 0)
                ),
                simd_float2(
                    x: Float(pointSpine2D?.x ?? 0),
                    y: Float(pointSpine2D?.x ?? 0)
                )
            )
    ...
    func computeOffsetOfRoot(observation: [HumanKeypointName : SCNNode]) -> CGPoint {
        var returnPoint = CGPoint(x: 0, y: 0)
        do {
            let point = observation[.Pelvis]?.simdPosition
            // Change to image scale.
            var xShift = Double(CGFloat(point?.x ?? 0) * imageNodeSize.width)
            // Recenter origin - translation shift left.
            xShift -= imageNodeSize.width / 2
            // Change to image scale.
            var yShift = Double(CGFloat(point?.y ?? 0) * imageNodeSize.height)
    ...
    func createCameraPyramidNode(observation: VNHumanBodyPose3DObservation) -> SCNNode {
        ...
        originCameraNode.geometry?.firstMaterial?.diffuse.contents = UIColor(ciColor: .cyan)
    ...
    func createSkeletonNodes(
        observation: [[[Float32]]]
    ) -> [HumanKeypointName: SCNNode] {

        var nodeJointDict = [HumanKeypointName: SCNNode]()
        
        for i in 0..<observation.count {
            do {
                let node = createNodeForRecognizedPoint(
                    point3D: simd_float3(x: 1/observation[i][0][0], y: 1/observation[i][1][0], z: 1/observation[i][2][0]),
                    jointGeometry: defaultNodeGeometry()
                )
                nodeJointDict.updateValue(node, forKey: PAMJointArr[i])
            } catch {
                print("Unable to return point: \(error).")
            }
        }
